var templateView = Backbone.View.extend({
    ...
    initData: function () {
        ...
        /*Drag drop with clone start*/

        this.currentCloneDraggableId ="";
        this.cloneArray = [];
        this.cloneCount = 0;
        /*Drag drop with clone end*/
    },
    ...
    postRender: function () {
        ...
        if(this.model.get('config').isDragDropWithClone){

            this.setOriginalPositionOfDraggables("#cloneDraggables-");
            this.initialiseCloneBucketArray();
            this.initialiseCloneArray();
            this.initialiseAssociativeArray();
    ...
    initialiseCloneBucketArray: function () {
        var bucketLength = this.model.get("bucketData").length;
        for (var i = 0; i < bucketLength; i++) {
            this.bucketArray.push({
                id: i + 1, 'name': 'bucketArray' + i, 'counter': 0, 'bucketArrayData': [{'dragId':0, 'dropId':0}], 'cloneArray':[], 'AnsCloneArray':[],'answerCloneArray':[],'finalAnswer':[],'isBucketLimitReached':false
            });
        }
    },
    initialiseCloneArray: function(){
        var dragLength = this.model.get("draggables").length;
    ...
    initialiseAssociativeArray : function(){
        var answerArrayLength = this.model.get("draggables").length;
        var bucketLength = this.model.get("bucketData").length;
        var indexValue;
        for(var i=0;i<bucketLength;i++){
            for(var k=0;k<answerArrayLength;k++){
                indexValue = "drag"+k;
                this.bucketArray[i].AnsCloneArray[indexValue] = 0;
            }
    ...
    onMouseDown: function (evt) {
        ...
        if(this.model.get('config').isDragDrop){
            this.initialDragObjectSettings(this.currentDraggableId,evt);
            this.BindEventsToClones(this.currentDraggableId);
        }
        if (this.model.get('config').isBucketDragDrop) {
            this.initialDragObjectSettings(this.currentDraggableId,evt);
        ...
        if(this.model.get('config').isDragDropWithClone){
            if(this.isDragging == true){
                this.DestroyCloneObject(this.currentCloneDraggableId);
            }
            var dragIdNo = this.currentDraggableId.split("-")[1];

            this.currentCloneDraggableId = "cloneDrag"+this.cloneArray[dragIdNo-1].cloneCount+"-"+dragIdNo;
            var pos = this.$el.find("#"+this.currentCloneDraggableId).position();

            if(pos == undefined){

                if($("#"+this.currentCloneDraggableId).hasClass('dragClass') && evt.currentTarget.id != "cloneDraggables-"+dragIdNo || $("#"+evt.currentTarget.id).hasClass("showSolution")){
                    return;
                }
                this.makeClone(this.currentDraggableId);
                this.initialDragObjectSettings(this.currentCloneDraggableId,evt);
                this.BindEventsToClones(this.currentCloneDraggableId);
                ...
                ++this.cloneArray[dragIdNo-1].cloneCount;
                this.currentCloneDraggableId = "cloneDrag"+this.cloneArray[dragIdNo-1].cloneCount+"-"+dragIdNo;
    ...
    //only for drag drop with clone
    makeClone:function(currentDraggableId){

        this.$el.find("#"+currentDraggableId).clone().appendTo("#dragContainer").addClass("cloneDrag");
        this.$el.find(".cloneDrag").removeAttr("id");
        this.$el.find(".cloneDrag").attr("id",this.currentCloneDraggableId);
    },

    initialDragObjectSettings:function(currentCloneDraggableId,evt){
        // console.log("#"+currentCloneDraggableId);
        this.isDragging =true;
        if(this.model.get('config').isDragDropWithClone){
            var position = this.$el.find("#"+currentCloneDraggableId).position();

            this.$el.find("#"+currentCloneDraggableId).css("position","absolute");
            this.$el.find("#"+currentCloneDraggableId).css("left",position.left);
            this.$el.find("#"+currentCloneDraggableId).css("top",position.top);


            this.orgPos = this.$el.find("#" + currentCloneDraggableId).position();
            this.parentPos = this.$el.find("#" + currentCloneDraggableId).parent().parent().position();
            //var parent = this.$el.find("#"+this.currentDraggableId).parents("#MainContainer3");
            this.parentSize = {width: this.$el.find("#" + currentCloneDraggableId).parent().parent().width(), height: this.$el.find("#" + currentCloneDraggableId).parent().parent().height()};
            //this.parentSize = {width:parent.width(), height:parent.height()};
            this.dragObjectSize = {width: this.$el.find("#" + currentCloneDraggableId).width(), height: this.$el.find("#" + currentCloneDraggableId).height()}


        }
        else{
            this.orgPos = this.$el.find("#" + currentCloneDraggableId).position();
            this.parentPos = this.$el.find("#" + currentCloneDraggableId).parent().position();
            //var parent = this.$el.find("#"+this.currentDraggableId).parents("#MainContainer3");
            this.parentSize = {width: this.$el.find("#" + currentCloneDraggableId).parent().width(), height: this.$el.find("#" + currentCloneDraggableId).parent().height()};
            //this.parentSize = {width:parent.width(), height:parent.height()};
            this.dragObjectSize = {width: this.$el.find("#" + currentCloneDraggableId).width(), height: this.$el.find("#" + currentCloneDraggableId).height()}
        }
    ...
    BindEventsToClones:function(CurrentDragId){
        if(this.model.get('config').isDragDropWithClone){
            var parentId = this.$el.find("#"+CurrentDragId).parent().parent().attr('id');
            ...
            var parentId = this.$el.find("#"+CurrentDragId).parent().attr('id');
            this.$el.find("#"+parentId).on('mousemove', _.bind(this.onMouseMove, this));
            this.$el.find("#"+parentId).on('mouseup', _.bind(this.onMouseUp, this));
        }
    },
    ...
    snapToPosition: function (position) {
        this.setStackOrder();
        if(this.model.get('config').isDragDropWithClone){
            this.$el.find("#"+this.currentCloneDraggableId).css('left', position.left);
            this.$el.find("#"+this.currentCloneDraggableId).css('top', position.top);
        }
        else{
    ...
    onMouseUp: function (e) {
        ...
        if (this.isDragging) {
            ...
            if(this.model.get('config').isDragDropWithClone){
                this.onCloneDragDropMouseUp();
            }
        }
    },
    onCloneDragDropMouseUp:function(){
        var $self = this;
        var $dropArea = null;

        this.$el.find('.dropCloneOptions').each(function () {
            var $target = $(this);
    ...
    onDrop: function ($dropArea) {
        ...
        if(this.model.get('config').isDragDropWithClone){
            this.onDropOfDragDropWithClone($dropArea)
        }
    },
    onDropOfDragDropWithClone : function($dropArea){

        if ($dropArea == null) {
            this.DestroyCloneObject();
        }
        else{
            ...
            this.snapToBucketObject($dropArea,this.currentCloneDraggableId);

            //console.log(this.currentCloneDraggableId);
            if (!this.$el.find('#' + this.currentCloneDraggableId).hasClass("dragClass")) {
                this.$el.find('#' + this.currentCloneDraggableId).addClass("dragClass").css("cursor", 'default');
            }
    ...
    onDropOfBucketDragDrop: function ($dropArea) {
        ...
        if ($dropArea == null) {
            ...
            this.snapToBucketObject($dropArea,this.currentDraggableId);
    ...
    snapToBucketObject: function ($dropArea, currentDraggableId) {
        this.bucketObjectPosition = $dropArea.position();

        var currentDraggableId = currentDraggableId.split("-")[1];
        var droppedOn = $dropArea.attr('id');
        var droppedId = droppedOn.split("-")[1];
        this.checkAssociativeArray[currentDraggableId] = droppedId;
        if(this.model.get('config').isDragDropWithClone){
            this.snapToPositionForDragDropWithClone({left: this.bucketObjectPosition.left, top: this.bucketObjectPosition.top}, $dropArea);
        }
        else{
    ...
    snapToPositionForDragDropWithClone:function(position, $dropArea) {
        for (var i = 0; i < this.bucketArray.length; i++) {
        ...
        var currentDraggableId = this.currentCloneDraggableId.split("-")[1];
        this.cloneCount = this.cloneArray[currentDraggableId-1].cloneCount;

        for (var i = 0; i < this.bucketArray.length; i++) {
            for(var j=1;j<=this.bucketArray[i].bucketArrayData.length;j++){
                if(this.bucketArray[i].bucketArrayData[j]!=undefined){
                    this.bucketArray[i].counter++;
                }
            }
        }

        //this.totalElementsInBucket[this.cloneArray[currentDraggableId-1].cloneCount] = droppedId;
        this.getPositionForDraggablesWithClone(currentDraggableId);
        this.pushDataInBucketArrayWithClone(droppedId, currentDraggableId, bucketId);
        this.cloneSettings();
    },

    getPositionForDraggablesWithClone: function (currentDraggableId) {
        ...
        for (var i = 0; i < this.bucketArray.length; i++) {
            for(var j = 1;j<=this.bucketArray[i].bucketArrayData.length;j++){
                ...
                if(this.bucketArray[i].bucketArrayData[j] != undefined){
                    ...
                    var currentDragging = this.bucketArray[i].bucketArrayData[j].dragId;
                    if(currentDragging != 0){

                        var currentHeight = this.$el.find("#cloneDrag"+this.cloneArray[dragIdNo-1].cloneCount+"-" + currentDragging).height();
                        var currentPosition = this.$el.find("#cloneDrag"+this.cloneArray[dragIdNo-1].cloneCount+"-" + currentDragging).position();

                        if( currentPosition != undefined){
                            var offsetTop = currentPosition.top + currentHeight + this.dragMargin;
                            this.$el.find("#"+this.currentCloneDraggableId).css('top', offsetTop);
                        }
    ...
    pushDataInBucketArrayWithClone: function (droppedId, currentDraggableId, bucketId) {
        ...
        for (var i = 0; i < bucketLength; i++) {
                ...
                var bucketDataArrayLength = this.bucketArray[i].bucketArrayData.length;
                if(droppedId == this.model.get("bucketData")[i].id && !this.bucketArray[i].isBucketLimitReached && this.bucketArray[i].bucketArrayData.length <= this.model.get("bucketData")[i].bucketLimit ){
                    this.bucketArray[i].bucketArrayData.push({'dragId':currentDraggableId, 'dropId':droppedId});
                    bucketCounter = this.bucketArray[i].counter;
                }
                else if(this.bucketArray[i].bucketArrayData.length > this.model.get("bucketData")[i].bucketLimit && droppedId == this.bucketArray[i].id){
                    this.bucketArray[i].isBucketLimitReached = true;
                    this.DestroyCloneObject();
                }
        ...
        if (bucketCounter <= 1) {
            leftOffset = (this.$el.find("#" + bucketId).position().left) + (this.$el.find("#" + bucketId).width() - this.$el.find("#" + this.currentCloneDraggableId).width()) / 2;
            topOffset = ((this.$el.find("#" + bucketId).height()) + (this.$el.find("#" + bucketId).position().top) - (this.$el.find("#" + this.currentCloneDraggableId).height())) - this.dragMargin;
        }
        else {
            leftOffset = (this.$el.find("#" + bucketId).position().left) + (this.$el.find("#" + bucketId).width() - this.$el.find("#" + this.currentCloneDraggableId).width()) / 2;
            topOffset = ((this.$el.find("#" + bucketId).height()) + (this.$el.find("#" + bucketId).position().top) - ((this.$el.find("#" + this.currentCloneDraggableId).height()) * bucketCounter)) - (this.dragMargin * bucketCounter);
        }
        this.$el.find("#" + this.currentCloneDraggableId).css('left', leftOffset);
        this.$el.find("#" + this.currentCloneDraggableId).css('top', topOffset);


    },

    cloneSettings:function(){
        this.$el.find("#"+this.currentCloneDraggableId).removeClass("cloneDrag");
        this.disableResetButton(false);
        var counter = 0;
        var bucketLength = this.model.get("bucketData").length;
        ...
        /*for(var i=0;i<bucketLength;i++){
            ...
            counter+=this.bucketArray[i].bucketArrayData.length
        }
        if(counter-bucketLength == this.model.get("config").totalLimitOfBucket){*/
    ...
    DestroyCloneObject:function(){

        this.$el.find("#"+this.currentCloneDraggableId).fadeOut(1000);
        this.$el.find("#"+this.currentCloneDraggableId).remove();
        var dragIdNo = this.currentDraggableId.split("-")[1];

        if(this.cloneArray[dragIdNo-1].cloneCount == 1){
            this.cloneArray[dragIdNo-1].cloneCount = 1
        }
        else{
            --this.cloneArray[dragIdNo-1].cloneCount;
        }
    ...
    hitTest: function ($target) {
        ...
        if(this.model.get('config').isDragDropWithClone){
            ...
            var getObjectPos = this.$el.find("#" + this.currentCloneDraggableId).position();
        }
        else{
        ...
        if(this.model.get('config').isDragDropWithClone){
            var srcRight = getObjectPos.left + (this.$el.find("#" + this.currentCloneDraggableId).width());
        }
        else{
        ...
        if(this.model.get('config').isDragDropWithClone){
            var srcBottom = getObjectPos.top + (this.$el.find("#" + this.currentCloneDraggableId).height());
        }
        else{
    ...
    validateAnswersForDragDropWithClone: function(){
        var bucketLength = this.model.get("bucketData").length;

        var answerArray = this.validateAnswerWithClone(bucketLength);

        var ans = this.returnAnswer(answerArray);
        this.$el.find(".dragOptions").addClass("showSolution");
        this.showFeedbackForDragDropWithClone(ans,answerArray);
    },

    returnAnswer:function(answerArray){
        var ans,element;
    ...
    validateAnswerWithClone:function(bucketLength){
        var AnswerArray = [];
        var self = this;
        var draggablesLength = this.model.get("draggables").length;
        for(var i=0;i<bucketLength;i++){
           var answerArray = this.model.get("bucketData")[i].answerArray;
            var answerLength = answerArray.length;
            for(var k=1; k<=draggablesLength; k++){
                for(var j=1;j<=this.bucketArray[i].bucketArrayData.length;j++){
                    if(this.bucketArray[i].bucketArrayData[j] != undefined){

                        if(k == parseInt(this.bucketArray[i].bucketArrayData[j].dragId)){
                            this.bucketArray[i].cloneArray.push(parseInt(this.bucketArray[i].bucketArrayData[j].dragId));
                        }
            ...
            this.sortCloneArray(this.bucketArray[i].cloneArray);

            var unique=this.uniqueAnswer(this.bucketArray[i].cloneArray);

            this.validationProcess(unique,answerLength,i);



            console.log(self.bucketArray[i].AnsCloneArray);
            for(var k=0;k<draggablesLength;k++){
                var drag = "drag"+k;
                this.bucketArray[i].answerCloneArray[k] = this.bucketArray[i].AnsCloneArray[drag];
            }

            this.bucketArray[i].finalAnswer = _.filter(this.bucketArray[i].answerCloneArray,function (answer, index) {

                if(answerArray[index] != 0){
                    return answer == answerArray[index];
                }

            },this);

            if(this.bucketArray[i].finalAnswer.length == 0){
                AnswerArray[i]=0;
            }
            else if(this.bucketArray[i].finalAnswer.length < this.model.get("bucketData")[i].correctAnsLength){
                AnswerArray[i]=1;
            }
            else{
                AnswerArray[i]=2;
    ...
    sortCloneArray:function(sortArray){
        sortArray.sort();
    },

    uniqueAnswer:function(a){
    ...
    validationProcess:function(unique,answerLength,i){
        var element;
        for(var k=0;k<answerLength;k++){
            if(this.bucketArray[i].cloneArray[k] != undefined){
                element = this.bucketArray[i].cloneArray[k];
                if(this.bucketArray[i].cloneArray[k] != this.bucketArray[i].cloneArray[k-1] && k != 0){
                    var indexToPushValue = this.bucketArray[i].cloneArray[k]-1;
                    indexToPushValue = "drag"+indexToPushValue;
                    this.bucketArray[i].AnsCloneArray[indexToPushValue]=this.countInArray(this.bucketArray[i].cloneArray,element);
                }
                else if(k == 0){
                    var indexToPushValue = this.bucketArray[i].cloneArray[k]-1;
                    indexToPushValue = "drag"+indexToPushValue;
    ...
    showFeedbackForDragDropWithClone:function(ans,answerArray){
        if (this.attemptCount < this.noOfAllowedAttempts) {
            if(ans == true && answerArray[0] == 0){
                this.disableResetButton(false);
                this.$el.find('#incorrect').show();
            }
            else if(ans == true && answerArray[0] == 1){
                this.disableResetButton(false);
                this.$el.find('#partial').show();
            }
            else if(ans == true && answerArray[0] == 2){
                this.disableResetButton(true);
                this.$el.find("#correct").show();
                this.disableSubmitButton(true);
            }
...
//changes by preeti
    ...
    showSolutionForDragDropWithClone:function(){
        this.onDragDropWithCloneReset();

        var bucketLength = this.model.get("bucketData").length;
        var draggableArray = this.model.get("draggables").length;
        for(var i=0;i<bucketLength;i++){
            var bucketId = "cloneDropArea-"+(i+1);
            for(var j=0;j<draggableArray;j++){
                var cloneCount= this.model.get("bucketData")[i].answerArray[j];
                var currentDraggableId = "cloneDraggables-"+(j+1);
                this.$el.find("#"+currentDraggableId).addClass("showSolution");
                for(var k=1;k<=cloneCount;k++){
                    this.currentCloneDraggableId = "cloneDrag"+k+"-"+(j+1)+(i+1);
                    this.makeClone(currentDraggableId);
                    this.$el.find("#"+this.currentCloneDraggableId).removeClass("cloneDrag");
                    var bucketCounter = ++this.bucketArray[i].counter;
                    if (bucketCounter <= 1) {
                        leftOffset = (this.$el.find("#" + bucketId).position().left) + (this.$el.find("#" + bucketId).width() - this.$el.find("#" + this.currentCloneDraggableId).width()) / 2;
                        topOffset = ((this.$el.find("#" + bucketId).height()) + (this.$el.find("#" + bucketId).position().top) - (this.$el.find("#" + this.currentCloneDraggableId).height())) - this.dragMargin;
                    }
    ...
    onSubmitButtonClicked: function () {
        ...
        else if(this.model.get('config').isDragDropWithClone){
            this.validateAnswersForDragDropWithClone();
        }
    ...
    onDragDropWithCloneReset:function(){
        for(var i=0;i<this.model.get("draggables").length;i++){
            var cloneCount = this.cloneArray[i].cloneCount;
            var dragIdNo = i+1;
            for(var j=1;j<=cloneCount;j++){
                $("#cloneDrag"+j+"-"+dragIdNo).remove();
            }
            this.cloneArray[i].cloneCount = 1;
        }
        for(var i=0;i<this.model.get("bucketData").length;i++){
            this.bucketArray[i].id = i+1;
            this.bucketArray[i].counter =0;
            this.bucketArray[i].cloneArray=[];
            this.bucketArray[i].AnsCloneArray=[];
            this.bucketArray[i].finalAnswer = [];
            this.bucketArray[i].isBucketLimitReached=false;
            this.bucketArray[i].bucketArrayData = [{'dragId':0, 'dropId':0}];
        }
        this.$el.find(".dragOptions").removeClass("showSolution");
        this.disableSubmitButton(true);
    },
    ...
    onShowSolButtonClicked: function () {
        ...
        if (this.model.get('config').isFillInTheBlank) {
            ...
            this.showSolutionForDragDropWithClone()
        }
